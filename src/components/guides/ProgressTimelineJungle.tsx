...
export const ProgressTimeline = ({ timeLine, storageKey, theme, pageHrefs }: ProgressTimelineProps) => {
    const [completedStates, setCompletedStates] = useState<boolean[]>([false, false, false, false]);
    const [hoverStates, setHoverStates] = useState<boolean[]>([false, false, false, false]);
    const [pressedStates, setPressedStates] = useState<boolean[]>([false, false, false, false]);
    ...
    const elements = [
        ...
        },
        {
            ...
            offset: -6,
            top: 0,
            uid: 'topLine-2' // Уникальный идентификатор
        },
        {
            type: 'flag',
            index: 3,
